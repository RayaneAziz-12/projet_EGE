...
$("#adresse").on("change", function(){
    ...
    if(adresse === ""){
        ...
        ADRESSVALIDATOR = false;
        $('.msgAdresse').html('Entrez votre adresse');
    }
    else{
...
$("#date_naissance").on("change", function(){
    var date_naissance = this.value;

    if(date_naissance === ""){
        ("#date_naissance").css('border','1px solid red');
        DATEVALIDATOR = false;
        $('.msgDateNaissance').html('Entrez une date svp');
    }
    else{
        ADRESSVALIDATOR = true;
        $("#date_naissance").css('border','1px solid green');
        $('.msgDateNaissance').html('');
    }
    checkForm();
})
